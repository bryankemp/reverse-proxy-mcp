...
class BackendListDialog extends StatelessWidget {
  ...
  Future<void> _showEditBackendForm(BuildContext context, backend) async {
    ...
    if (result == true) {
      ...
      if (ok) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Backend updated')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _showCreateBackendForm(BuildContext context) async {
    ...
    if (result == true) {
      ...
      final provider = context.read<BackendProvider>();
      final ok = await provider.createBackend(
        name: name,
        host: host,
        port: port,
        protocol: protocol,
      ...
      if (ok) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Backend created')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      itemBuilder: (context) => [
                        PopupMenuItem(
                          ...
                          onTap: () {
                            Future.delayed(
                              Duration.zero,
                              () => _showEditBackendForm(context, backend),
                            );
                          },
                        ),
                        PopupMenuItem(
                        ...
                        PopupMenuItem(
                          ...
                          onTap: () async {
                            ...
                            if (confirm == true) {
                              final ok = await provider.deleteBackend(
                                backend.id,
                              );
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      ok ? 'Backend deleted' : 'Delete failed',
                                    ),
                                  ),
                                );
                              }
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildDetailRow(
                              'Created By',
                              'User ID: ${backend.createdBy}',
                            ),
                            _buildDetailRow(
                              'Created At',
                              backend.createdAt.toString(),
                            ),
                            _buildDetailRow(
                              'Updated At',
                              backend.updatedAt.toString(),
                            ),
                          ],
                        ),
                      ),
  ...
  Future<void> _showEditRuleForm(BuildContext context, rule) async {
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    items: backends.map((backend) {
                      return DropdownMenuItem<int>(
                        value: backend.id,
                        child: Text(
                          '${backend.name} (${backend.protocol}://${backend.host}:${backend.port})',
                        ),
                      );
                    }).toList(),
    ...
    if (result == true) {
      ...
      if (domain.isEmpty || selectedBackendId == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Domain and Backend are required')),
        );
        return;
      }
      ...
      if (ok) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Rule updated')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(provider.error ?? 'Update failed')),
        );
  ...
  Future<void> _showCreateRuleForm(BuildContext context) async {
    ...
    if (backends.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No active backends available. Please create a backend first.',
          ),
        ),
      );
      return;
    ...
    if (result == true) {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(ok ? 'Rule created' : 'Create failed')),
      );
    }
  }
                      ...
                      itemBuilder: (context) => [
                        PopupMenuItem(
                          ...
                          onTap: () {
                            Future.delayed(
                              Duration.zero,
                              () => _showEditRuleForm(context, rule),
                            );
                          },
                        ),
                        PopupMenuItem(
                          ...
                          onTap: () async {
                            final confirm = await showDialog<bool>(
                              ...
                              builder: (ctx) => AlertDialog(
                                title: const Text('Confirm Deletion'),
                                content: Text(
                                  'Delete rule for "${rule.domain}" permanently? This cannot be undone.',
                                ),
                                actions: [
                                  TextButton(
                                    onPressed: () => Navigator.pop(ctx, false),
  ...
  Future<void> _showCreateCertForm(BuildContext context) async {
    ...
    if (result == true) {
      ...
      if (domain.isEmpty) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Domain is required')));
        return;
      }
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(ok ? 'Certificate saved' : 'Save failed')),
      );
    }
  }
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () async {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            ok ? 'Certificate deleted' : 'Delete failed',
                          ),
                        ),
                      );
                    },
