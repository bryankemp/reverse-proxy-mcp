...
def setup_logging() -> None:
    ...
    file_handler = logging.FileHandler(log_dir / "nginx_manager.log")
...
def log_request(method: str, path: str, user_id: int | None = None, **kwargs: Any) -> None:
    ...
    """
    logger = get_logger("nginx_manager.api")
...
def log_db_query(
    ...
    """
    if settings.debug:
        logger = get_logger("nginx_manager.db")
        msg = f"Query: {query}"
        if params:
            msg += f" | Params: {params}"
        if duration_ms:
...
def log_nginx_operation(operation: str, success: bool, message: str = "") -> None:
    ...
    """
    logger = get_logger("nginx_manager.nginx")
